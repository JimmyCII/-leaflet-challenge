...
function createMap(earthquakes) {
    ...
    legend.onAdd = function () {
        ...
        let depths = [-10, 10, 30, 50, 70, 90, 110, 130]; // Example depth values
        let labels = [];


        div.innerHTML += "<h4>Depth (km)</h4>"; // Added a title for depth
    ...
    d3.json(plateUrl).then(function (plateData){
         let tectonicPlates = L.geoJSON(plateData, {
          ...
          overlayMaps.TectonicPlates = tectonicPlates;
        L.control.layers(baseMaps, overlayMaps, {
            collapsed: false
        }).addTo(myMap);
    })
